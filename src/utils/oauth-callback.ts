...
import { getDb, updateDiscordMetadata } from "./database.ts";
import { describeError, recordInteractionError } from "./interaction-errors.ts";
import { hasSocialLayerScope } from "./lobby-oauth.ts";
import { connectedPage, failedPage } from "./oauth-pages.ts";

/**
 * The OAuth2 callback flow: exchange the code, store the tokens, update the
 * linked-role metadata.
 *
 * Deliberately not `mini.discordOAuthCallback()`: that helper renders every
 * branch — including its own error path — by reading an HTML file at request
 * time, and those files are not in the function bundle, so the endpoint answered
 * `FUNCTION_INVOCATION_FAILED` and could never report why.
 *
...
export function sendHtml(res: OAuthResponse, html: string): void {
	res.statusCode = 200;
...
/** Reads one cookie from the raw header (the runtime ships no cookie parser). */
export function readCookie(req: OAuthRequest, name: string): string | null {
...
export async function handleOAuthCallback(
	...
	try {
		...
		// Recorded so `/api/diag` can explain it: this is a failure the user
